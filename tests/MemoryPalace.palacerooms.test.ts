...
describe('MemoryPalace - PalaceRoom Integration', () => {
  ...
  describe('content association with palace rooms', () => {
    ...
    describe('drawings', () => {
      ...
      test('should remove drawing from palace room', () => {
        ...
        const removed = palace.removeDrawingFromPalaceRoom(roomId, 'test-drawing');
    ...
    describe('codebase views', () => {
      ...
      test('should not add non-existent view', () => {
        const added = palace.addCodebaseViewToPalaceRoom(roomId, 'non-existent');
        expect(added).toBe(false);
      });
      ...
      test('should remove codebase view from palace room', () => {
        ...
        const removed = palace.removeCodebaseViewFromPalaceRoom(roomId, 'test-view');
        expect(removed).toBe(true);
    ...
    describe('notes', () => {
      ...
      test('should remove note from palace room', () => {
        ...
        const removed = palace.removeNoteFromPalaceRoom(roomId, noteResult.note.id);
        expect(removed).toBe(true);
  ...
  describe('complex scenarios', () => {
    test('should prevent deleting room with content', () => {
      const createResult = palace.createPalaceRoom({ name: 'Room with Content' });
      if (!createResult.palaceRoom) {
        throw new Error('Failed to create room');
      }
      const roomId = createResult.palaceRoom.id;
      ...
      // Add a drawing
      palace.saveDrawing('important-drawing', JSON.stringify({ data: 'important' }));
