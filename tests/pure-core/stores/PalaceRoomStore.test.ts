...
describe('PalaceRoomStore', () => {
  ...
  describe('initialization', () => {
    test('should create palace-rooms directory on init', () => {
      const palaceRoomsPath = fs.join(alexandriaPath, 'palace-rooms');
      expect(fs.exists(palaceRoomsPath)).toBe(true);
    });

    test('should create default room on init', () => {
      const defaultRoom = store.getRoom('default');
  ...
  describe('listRooms', () => {
    ...
    test('should handle empty palace rooms directory', () => {
      ...
      const emptyStore = new PalaceRoomStore(emptyFs, '/empty/repo/.alexandria' as ValidatedAlexandriaPath);

      const rooms = emptyStore.listRooms();
      expect(rooms.length).toBe(1); // Only default room
    });
  });
  ...
  describe('updateRoom', () => {
    ...
    test('should allow updating default room description', () => {
      const result = store.updateRoom('default', { description: 'Updated description' });
      expect(result.success).toBe(true);
  ...
  describe('getDefaultRoom', () => {
    ...
    test('should recreate default room if deleted somehow', () => {
      ...
      const defaultRoomPath = fs.join(alexandriaPath, 'palace-rooms', 'default.json');
