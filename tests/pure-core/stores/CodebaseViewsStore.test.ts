...
describe("Pure CodebaseViewsStore", () => {
  ...
  it("should return false when updating non-existent view", () => {
    const updated = store.updateView(validatedRepoPath, "non-existent", {
      name: "New Name",
    });
...
describe("generateViewIdFromName", () => {
  it("should convert names to URL-safe IDs", () => {
    expect(generateViewIdFromName("My Architecture View")).toBe(
      "my-architecture-view",
    );
    expect(generateViewIdFromName("Frontend (React + Redux)")).toBe(
      "frontend-react-redux",
    );
    expect(generateViewIdFromName("  Spaces  ")).toBe("spaces");
    expect(generateViewIdFromName("Special!@#$%Characters")).toBe(
      "special-characters",
    );
  });
