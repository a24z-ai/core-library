...
import {
  ...
  getNotesForPath,
  getTagDescriptions,
} from "../dist/core-mcp/store/notesStore.js";
...
async function main() {
  ...
  saveTagDescription(
    repoPath,
    "bug-fix",
    "Issues that were fixed in the codebase",
  );
  saveTagDescription(
    repoPath,
    "authentication",
  ...
  saveTagDescription(
    repoPath,
    "security",
    "Security-related implementations and fixes",
  );
  ...
  console.log(
    `   - Notes with 'bug-fix' tag: ${initialNotes.filter((n) => n.tags.includes("bug-fix")).length}`,
  );

  const initialTags = getTagDescriptions(repoPath);
  console.log(
    `   - Tag descriptions: ${Object.keys(initialTags).join(", ")}\n`,
  );
  ...
  console.log(
    `   ℹ️  Description action: ${response2.results.descriptionAction}\n`,
  );
