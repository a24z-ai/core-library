...
export interface ReadingRecordAdapter {
  ...
  getLibraryCard(userId: string): Promise<StorageResult<AlexandriaLibraryCard | null>>;
  saveLibraryCard(libraryCard: AlexandriaLibraryCard): Promise<StorageResult<void>>;
  ...
  updateVisit(visitId: string, updates: Partial<AlexandriaVisit>): Promise<StorageResult<void>>;
  deleteVisit(visitId: string): Promise<StorageResult<void>>;
  queryVisits(query: VisitQuery): Promise<StorageResult<AlexandriaVisit[]>>;
  
  // Bookmark Management
  createBookmark(visitId: string, bookmark: AlexandriaBookmark): Promise<StorageResult<AlexandriaBookmark>>;
  getBookmark(bookmarkId: string): Promise<StorageResult<AlexandriaBookmark | null>>;
  updateBookmark(bookmarkId: string, updates: Partial<AlexandriaBookmark>): Promise<StorageResult<void>>;
  deleteBookmark(bookmarkId: string): Promise<StorageResult<void>>;
  queryBookmarks(query: BookmarkQuery): Promise<StorageResult<AlexandriaBookmark[]>>;
...
export interface StorageEvents {
  ...
  'bookmark:updated': { bookmarkId: string; changes: Partial<AlexandriaBookmark> };
