...
export class MemoryReadingRecordAdapter implements ReadingRecordAdapter {
  ...
  async queryBookmarks(
    ...
    if (query.visitId) {
      ...
      const matchingVisits = Array.from(this.visits.values()).filter(
        (visit) => {
          return (
            (!query.volumeId || visit.volumeId === query.volumeId) &&
      ...
      bookmarks = bookmarks.filter((bookmark) =>
        matchingBookmarkIds.has(bookmark.id),
      );
    }
  ...
  async import(jsonData: string): Promise<StorageResult<void>> {
    try {
      ...
      if (data.libraryCards) {
        data.libraryCards.forEach(
          ([id, card]: [string, AlexandriaLibraryCard]) => {
      ...
      if (data.bookmarks) {
        data.bookmarks.forEach(
          ([id, bookmark]: [string, AlexandriaBookmark]) => {
