...
export class LocalStorageReadingRecordAdapter implements ReadingRecordAdapter {
  ...
  async saveLibraryCard(libraryCard: AlexandriaLibraryCard): Promise<StorageResult<void>> {
    try {
      ...
      const cardToSave: SerializedLibraryCard = {
        ...libraryCard,
        activeVisits: libraryCard.activeVisits instanceof Map 
  ...
  async deleteVisit(visitId: string): Promise<StorageResult<void>> {
    try {
      ...
      const bookmarksToDelete = Object.entries(data.bookmarks)
        .filter(([, bookmark]) => data.visits[visitId]?.bookmarks.some(b => b.id === bookmark.id))
        .map(([id]) => id);
    ...
    try {
      ...
      if (query.volumeId || query.chapterId) {
        ...
        bookmarks = bookmarks.filter(bookmark => {
          return (!query.volumeId || bookmark.volumeId === query.volumeId) &&
  ...
  async cleanup(olderThan: Date): Promise<StorageResult<number>> {
    try {
      ...
      const visitsToDelete = Object.entries(data.visits)
        .filter(([, visit]) => visit.startedAt < cutoffTime && !visit.isActive);
  ...
  private saveData(data: LocalStorageData): void {
    localStorage.setItem(`${this.keyPrefix}reading_records`, JSON.stringify(data));
  }
