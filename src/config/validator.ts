...
export class ConfigValidator {
  validate(config: unknown): ValidationResult {
    ...
    if (cfg.reporting !== undefined) {
      if (!this.isObject(cfg.reporting)) {
        ...
        this.validateReporting(
          cfg.reporting as Record<string, unknown>,
          errors,
        );
      }
    }
    ...
    const unknownKeys = Object.keys(cfg).filter(
  ...
  private validateContext(
    ...
    if (context.patterns !== undefined) {
      if (!this.isObject(context.patterns)) {
        ...
        if (
          patterns.include !== undefined &&
          !Array.isArray(patterns.include)
        ) {
          errors.push({
    ...
    if (
      context.useGitignore !== undefined &&
      typeof context.useGitignore !== "boolean"
    ) {
      errors.push({
    ...
    if (
      context.followSymlinks !== undefined &&
      typeof context.followSymlinks !== "boolean"
    ) {
      errors.push({
  ...
  private validateReporting(
    ...
    if (
      reporting.verbose !== undefined &&
      typeof reporting.verbose !== "boolean"
    ) {
      errors.push({
